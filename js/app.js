...
async function bootstrap() {
  try {
    ...
    state.premiumActive = Boolean(
      sessionResponse.premiumActive ||
      bootstrapResponse?.premiumActive ||
      readCookie('premium_hint') === '1'
    );
  } catch {
...
function syncCustomLanguageMenu(locale) {
  ...
  const selected =
    Array.from(el.language.options).find((option) => option.value === locale) ||
    el.language.options[0];

  el.languageCurrent.textContent = selected ? selected.textContent : locale;

  el.languageOptions.innerHTML = Array.from(el.language.options)
    .map(
      (option) => `
...
function syncSessionUI() {
  if (el.premiumBadge) {
    el.premiumBadge.classList.toggle('hidden', !state.premiumActive);
  }

  if (el.removeBgBtn) {
    el.removeBgBtn.disabled = false;
    el.removeBgBtn.textContent = state.premiumActive
      ? t(state.locale, 'removeBg')
      : t(state.locale, 'removeBgLocked');
    el.removeBgBtn.classList.toggle('is-locked', !state.premiumActive);
  }

  if (el.checkoutBtn) {
    el.checkoutBtn.disabled = state.premiumActive;
    el.checkoutBtn.textContent = state.premiumActive
      ? t(state.locale, 'premiumEnabledCta')
      : t(state.locale, 'buyPremium');
    el.checkoutBtn.classList.toggle('is-active', state.premiumActive);
  }

  if (el.checkoutStatus) {
    el.checkoutStatus.textContent = state.premiumActive
      ? t(state.locale, 'premiumEnabledHint')
      : t(state.locale, 'checkoutIdle');
  }

  if (el.adSlots?.length) {
    el.adSlots.forEach((slot) => slot.classList.toggle('hidden', state.premiumActive));
  }

  if (state.humanVerified) {
    setHumanStatus(t(state.locale, 'humanVerified'), 'ok');
  } else {
    setHumanStatus(t(state.locale, 'humanPending'), 'warn');
  }

  if (state.premiumActive) {
    document.cookie = 'premium_hint=1; path=/; max-age=86400; SameSite=Lax';
  } else {
    document.cookie = 'premium_hint=0; path=/; max-age=0; SameSite=Lax';
  }
}

function bindEvents() {
  el.language?.addEventListener('change', (e) => setLocale(e.target.value));
  el.languageTrigger?.addEventListener('click', (e) => {
    e.stopPropagation();
    toggleLanguageMenu();
  });

  document.addEventListener('click', (e) => {
    if (!el.languageMenu?.contains(e.target)) closeLanguageMenu();
  });
...
async function onExport() {
  if (!state.analysis || !state.workingImage) {
  ...
  renderList(
    el.exportFactsList,
    facts.map((label) => ({ state: 'pass', label }))
  );
...
async function onRemoveBackground() {
  ...
  try {
    ...
    state.workingImage = image;
    state.file = new File([blob], 'background-cleaned.png', {
    ...
    window.LemonSqueezy.Setup({
      eventHandler: async (event) => {
        ...
        try {
          ...
          if (el.checkoutStatus) {
            el.checkoutStatus.textContent = t(state.locale, 'paymentCompleted');
          }

          if (el.analysisSummary) {
            el.analysisSummary.textContent = t(state.locale, 'premiumUnlocked');
          }
        } catch (error) {
          if (el.checkoutStatus) {
            el.checkoutStatus.textContent =
              error.message || t(state.locale, 'premiumActivationFailed');
          }
        }
      }
    });
