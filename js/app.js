...
import { createCheckout, removeBackground, getSessionStatus, activatePremium } from './premium.js';
...
async function bootstrap() {
  ...
  if (successSessionId && window.location.pathname.endsWith('/success.html')) {
    await handleSuccessPage(successSessionId);
  }
}
...
function syncSessionUI() {
  ...
  if (el.removeBgBtn) el.removeBgBtn.disabled = !state.premiumActive;
...
async function onRemoveBackground() {
  ...
  el.analysisSummary.textContent = 'Removing background…';
  try {
    ...
    if (el.originalPreview) el.originalPreview.src = url;
    renderFitImage(image);
    el.analysisSummary.textContent = 'Background removed. Run analysis again to refresh the crop.';
  } catch (error) {
...
async function handleSuccessPage(sessionId) {
  ...
  try {
    const data = await activatePremium(sessionId);
